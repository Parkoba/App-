...
<template>
    <div class="proceed-plane flex justify-end items-center px-5 mt-[30px]">
        <Transition name="intro-button" mode="out-in">
            <button class="bg-pb-950 text-white text-bold text-16 px-4 py-6 tracking-wider rounded-md" v-if="isSlideEnd" @click="router.navigate('/register', 'forward', 'push')">Get Started</button>
            <button class="bg-pb-950 text-white text-bold text-16 px-6 py-5 tracking-widest rounded-md" v-else @click="swiperEl.slideNext();">
                Next
            </button>
        </Transition>
    </div>
